...
-   Consiste à entraîner des modèles informatiques à effectuer des
    tâches sans avoir été explicitement programmés pour les accomplir

-   Les modèles peuvent s'améliorer au fil du temps en apprenant à
    partir de données
...
-   hypothetico-déductive (part d'hypothèses et utilise des tests
    statistiques pour les vérifier)
...
-   inductive (part de données et essaie de déduire les règles
    sous-jacentes)

-   peut être utilisé avec des données de grande taille et non
    structurées

-   modèles complexes et difficiles à interpréter (réseaux de neurones,
    arbres de décision)
:::
:::
...
-   Le jeu de données contient des exemples d'entrée et de sortie
    souhaités
-   L'objectif: généraliser apprentissage à partir de ces exemples pour
    prédire la sortie correcte pour de nouvelles entrées

# Apprentissage supervisé

$$\hat{y} = f(x, \theta)$$

où $x$ est l'entrée, $\theta$ sont les paramètres du modèle et $\hat{y}$
est la valeur prédite par le modèle pour l'entrée $x$.

Objectifs : Trouver les valeurs optimales de $\theta$ qui minimisent
l'erreur entre les valeurs prédites et valeurs réelles.
...
-   Découvrir une structure au sein d'un ensemble d'individus
    caractérisés par des covariables X
...
Objectifs : trouver des structures ou des patterns dans les données qui
peuvent être utilisés pour effectuer des tâches utiles

Les paramètres du modèle sont mis à jour en utilisant une fonction de
coût et une méthode d'optimisation afin de trouver des structures ou des
...
On cherche $\hat{\theta} = \binom{\hat{a}}{\hat{b}}$ où
$\hat{y} = \hat{a}\cdot x+\hat{b}$ pour que $\hat{y}$ soit le plus
proche de $y$
...
On trace une ligne, on mesure la qualité de la ligne tracé, on modifie
la ligne pour améliorer ça qualité et on répète
...
-   Simple à calculer et à interpréter, mais sensible aux outliers et
    peu robuste face à la skewness des données
...
-   Moins sensible aux outliers que le MSE, mais moins intuitive à
    interpréter
...
-   Simple à calculer, mais ne prend pas en compte la probabilité des
    prédictions
