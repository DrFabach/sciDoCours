...
-   Information Gain:
    $IG(D_{p},f) = I(D_{p})-\sum_{j=1}^{m} \frac{N_j}{N_p}I(D_j)$
...
-   construction en divisant récursivement en fonction de la
    caractéristique qui donne les sous-ensembles les plus purs
...
-   jusqu'à ce qu'un certain critère d'arrêt soit atteint : profondeur
    max, nobre minnimum de données par noeud
...
L'impureté de Gini : probabilité qu'un élément choisi au hasard dans le
nœud soit mal classé.
...
$$ Gini(jaune) = 1 - \sum_{i=1}^{n}P(i|rouge)^2 =\\ 1 - 0^2 - 0^2-1^2-0^2 = 1 $$
...
-   Post-élagage : Construction de l'arbre dans son intégralité puis
    calcul de l'intérêt de chaque nœud en partant des nœud terminaux -\>
...
Science](https://miro.medium.com/max/1200/1*RI9_LbQ5A9D1izBaU1ypzQ.gif){alt="Interpretability and Random Forests | by Tom Grigg | Towards Data Science"}
...
-   Contraindre la valeur des estimateurs des moindre carrés pour
    réduire la variance
...
-   Noyau polynomial:
    ...
    où $c$ est des hyperparamètres de décalage et $d$ est le degré du
    polynôme.
...
Rappel sur la profondeur des arbres : - petite : biais $\searrow$,
...
Estimation possible de l'erreur sur les sujets non sélectionnés par le
boosting
...
![](images/image-660090245.png)

# Kmean clustering

![![machine
learning](https://i0.wp.com/datascientest.com/wp-content/uploads/2020/05/kmeans-2.png?fit=377%252C368&ssl=1){alt="machine learning"}](https://i0.wp.com/datascientest.com/wp-content/uploads/2020/05/kmeans1.png?fit=377%252C368&ssl=1){alt="kmeans"}
...
## ![](https://delladata.fr/wp-content/uploads/2020/08/Q7HIP.gif){alt=""}
