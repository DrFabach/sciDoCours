...
```{r}
BDD_entrainement$pclass<- as.factor(BDD_entrainement$pclass)
...
fit<- glm(survived~. ,BDD_entrainement[!folds==10,], family = "binomial")
summary(fit)
```
...
```{r}
predictions<- predict(fit, BDD_entrainement[!folds==10,], type="response")
```
...
```{r}
table(predictions>0.5, BDD_entrainement$survived[!folds == 10])
```
...
```{r}
library(pROC)
roc<-roc( BDD_entrainement$survived[!folds==10],predictions )
(roc)
plot(roc)

coords(roc,seq(0,1,0.1))
...
caract_test(predictions>0.5, BDD_entrainement$survived[!folds==10]==1 )

caract_test(predictions>0.38, BDD_entrainement$survived[!folds==10] ==1)
```
...
res_data <- data.frame()
for (i in 1:10) {
fit_i<- glm(survived~., family = "binomial", BDD_entrainement[!fold==i,])
preditions_i<- predict(fit_i, BDD_entrainement[fold==i,],type= 'reponse') 
